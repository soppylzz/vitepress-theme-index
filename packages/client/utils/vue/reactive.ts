...
import { clearObject, hasOwnProperty, runtimeLogger } from "@vitepress-theme-index/shared";
...
function useSplitRefs<T extends object>(val: MaybeRefOrGetter<T | undefined>) {
  ...
  function checkKeys(target: object) {
    for (const key of initialKeys) {
      if (!(key in target)) {
        runtimeLogger.error(`missing key: ${key}, do not modify structure!`);
      }
    }
    if (Object.keys(target).length !== initialKeys.length) {
      runtimeLogger.error(`key count changed, do not modify structure!`);
    }
  }
...
export { useCachedComputed, useSplitRefs, useReactiveProxy };
